...
from fairseq.modules.adv_transformer_layer import (
    AdvTransformerDecoderLayer as TransformerDecoderLayer,
    AdvTransformerEncoderLayer as TransformerEncoderLayer,
)
from fairseq.user.xtransformer import Args, Task, XTransformerModel
...
@register_model('adv_transformer_lm')
class AdvTransformerModel(FairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--layer-sampler', default="last",
                            help="the way to sample encoder LM layers")
        # fmt: on
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, embed=None, **kwargs):
        """
        ...
        if embed is None:
            ...
            encoder_out = self.encode_forward(src_tokens, src_lengths=src_lengths, embed=embed, **kwargs)
    ...
    def encode_forward(self, src_tokens, src_lengths, cls_input=None, return_all_hiddens=True, embed=None, **unused):
        if self.encoder.embed_positions is not None:
            x = embed + self.encoder.embed_positions(src_tokens)
        if self.encoder.layernorm_embedding:
            x = self.layernorm_embedding(x)
        x = F.dropout(x, p=self.encoder.dropout, training=self.training)

        if self.encoder.layer_wise_attention:
        ...
        for layer in self.encoder.layers:
            ...
            if not self.training or (dropout_probability > self.encoder.encoder_layerdrop):
...
class LMEmbedding(nn.Module):
    def __init__(self, args, path):
        ...
        model_args = Args()
        dicts = {args.source_lang: src_dict, args.target_lang: tgt_dict}
        task = Task(dicts)
        model = XTransformerModel.build_model(model_args, task)
        state = torch.load(path)
        model.load_state_dict(state['model'], strict=False)
        model: XTransformerModel
        self.model = model.encoders[args.source_lang]
        self.model.eval()
        ...
        self.layer_sampler = args.layer_sampler
    ...
    def forward(self, src_tokens):
        ...
        hiddens = output["encoder_out"].transpose(0, 1)
        return hiddens
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if 'adaptive' in args.admin_init_type and not args.encoder_normalize_before:
            print('adaptive init')
            self.layers.extend([
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.layers = nn.ModuleList([])
        if 'adaptive' in args.admin_init_type and not args.decoder_normalize_before:
            print('adaptive init')
            self.layers.extend([
...
@register_model_architecture('adv_transformer_lm', 'adv_transformer_lm')
def base_architecture(args):
    ...
    args.layer_sampler = getattr(args, "layer_sampler", "last")

@register_model_architecture('adv_transformer_lm', 'lm_emb')
def lm_emb(args):
