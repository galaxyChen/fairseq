...
class AdvTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, args, LayerNum=None):
        ...
        if LayerNum is not None and not self.normalize_before:
            ...
            #adopt RPR
            if self.args.max_relative_length == -1:
                ...
                self.self_attn = RelativeMultiheadAttention(
                    self.embed_dim, args.encoder_attention_heads,
                    self.args.max_relative_length, dropout=args.attention_dropout, k_only=self.args.k_only,
                )
            ...
            if 'adaptive-profiling' == args.admin_init_type:
                ...
                # next_value = float(tmp_file.readlines()[LayerNum*2+0])
                print('encoder attn ratio: {}'.format(next_value))
                self.attention_ratio_change = nn.Parameter(torch.ones(self.embed_dim))
                self.attention_ratio_change.data.fill_(next_value)

                next_value = float(tmp_file.readline())
                # next_value = float(tmp_file.readlines()[LayerNum * 2 + 1])
...
class AdvTransformerDecoderLayer(nn.Module):
    """Decoder layer block.
